...
async function buildIndex(): Promise<{ blog: Available; project: Available }> {
	...
	for (const p of allBlog) {
		...
		const slug =
			p.data.canonicalSlug ??
			p.id.replace(/\.en\.(md|mdx)$/, "").replace(/\.en$/, "");
 ...
 * exists in the build. For static pages (home, /blog, hubs, etc.) always
 ...
 */
export async function alternateExists(
	pathname: string,
	currentLang: Lang,
): Promise<boolean> {
